...
// [[Rcpp::export]]
NumericVector fit_functionDSE_cpp(double N1, double N2, double D01, double D02,
				  double Di, NumericVector x) {
  int n = x.size();
  NumericVector y(n);

  for (int i = 0; i < n; ++i) {
    double val = -(x[i] + Di);
    double exp1 = 1.0 - std::exp(val / D01);
    double exp2 = 1.0 - std::exp(val / D02);
    y[i] = N1 * exp1 + N2 * exp2;
  }
