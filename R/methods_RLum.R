...
#' is given, e.g. `plot.RLum.Data.Curve` can be called by `plot(object, ...)`, where
...
#' underlying `RLum`-object structure of the package. The practical outcome is that
...
#'
#' @param x [RLum-class] or [Risoe.BINfileData-class] (**required**): input opject
#'
#' @param object [RLum-class] (**required**): input opject
#'
...
#'
#' @param i [character] *(optional)*: name of the wanted record type or data object or row in the `RLum.Data.Curve` object
#'
#' @param j [integer] *(optional)*: column of the data matrix in the `RLum.Data.Curve` object
#'
#' @param value [numeric] **(required)**: numeric value which replace the value in the
#' `RLum.Data.Curve` object
#'
#' @param drop [logical] *(with default)*: keep object structure or drop it
#'
#' @param subset `[subset]` [expression] (**required**): logical expression indicating elements or rows to keep,
#' this function works in [Risoe.BINfileData-class] objects like [subset.data.frame], but takes care
#' of the object structure
#'
#' @param row.names [logical] *(with default)*: enables or disables row names (`as.data.frame`)
#'
#' @param recursive [logical] *(with default)*: enables or disables further subsetting (`unlist`)
#'
...
#'
#' @note `methods_RLum` are not really new functions, everything given here are mostly just
...
#' be disabled, to just set the column `SET` to `TRUE` or `FALSE`
#' 
#' @md
#' @export
