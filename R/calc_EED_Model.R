...
# Helper functions ----------------------------------------------------------------------------
  ...
  .guess_EED_parameters <- function(
    ...
    ##TODO add option for control plot
    # par(mfrow = c(2,1))
...
if(is.null(kappa) || is.null(sigma_distr)){
  ...
  if(verbose)
    cat("\n>> Starting adpation for 'kappa' and 'sigma' ... ")
...
if(plot) {
  ...
  hist(
    ...
    main = "Distribution De: ADD HERE SAMPLE NAME",
  ...
  hist(
    ...
    border = rgb(0.7,0.1,0,.5),
    col = rgb(240,154,149,alpha = 255, maxColorValue = 255),
    main = "Distribution SE(De): ADD HERE SAMPLE NAME",
    xlab = plot_settings$xlab
  )
...
# # INITIALISATION
#
# # changer le r?pertoire courant selon le cas #
#
# # introduire le nom de fichier sans extension entre les guillemets de la ligne de commmande suivante
# file_name_head = "BDX22124_30"
#
# file_name = paste(file_name_head,".csv", sep = "")
# data <- read.csv(file_name, header=TRUE, sep = ";") # attention au s?parateur : sep = "," pour une virgule
# #                           sep = "\t" pour une tabulation
# #                           sep = ";" pour un point-vigule etc.
#
# calc_EED_Model(data = data, kappa = NULL, sigma_distr = NULL)
