...
not explain the theoretical and physical background. Please see Kreutzer et al. (in press), for details. 
...
sequence is based on the suggestions made by Kreutzer et al. (in press). To import the measurement data run the function 
...
```{r, Fig2, fig.cap="Graphical function output of the crosstalk data analysis. Please note that the plot shows the results from repeated cross-talk measurements as described by Kreutzer et al. (press).", fig.align="center", fig.height=6.5, fig.width=8, fig.pos="h", out.height="400px", out.width="500px", dpi=75}
...
An additional curve selection is not necessary, so far the sequence follows the suggestion by Kreutzer et al. (in press)
...
Gechromometrie 45, 56-67. [doi: 10.1515/geochr-2015-0086](http://dx.doi.org/10.1515/geochr-2015-0086)
