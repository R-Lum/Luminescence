...
The following tutorial assumes that all measurements have been performed on a Freiberg Instruments *lexsyg SMART* luminescence reader (Richter et al., 2015). Nevertheless, the general procedure should work also for other luminescence readers. However, please keep in mind that for readers without a separate measurement chamber the determination 
of the cross talk becomes indispensable and should not be skipped.  
...
Modify this argument if the selection here does not fit to your equipment. 
...
For this example six Al$_{2}$O$_{3}$:C pellets have been measured. The function returns an object
...
Usually, the data set to analyse contains pellets, which were used as travel dosimeters, i.e. this 
pellets were reset in the field at the time the other dosimeters were taken and then transported
along with the field dosimeters. The dose of the travel dosimeters is subtracted from the determined
dose of the field dosimeters. This can be done manually or automatically using the argument `travel_dosimeter` in the function `analyse_Al2O3C_Measurement()`. For the data set given above the analysis can be used 
...
The transform the values from the time domain to the dose domain, the function `Second2Gray()` 
can be used. 

# Ease your workflow

The analysis steps for the irradiation time correction and the irradiation cross-talk estimation 
might do not wanted to be repeated for every analysis. Thus, **R** allows the save results 
...
Dietze, M., Kreutzer, S., Burow, C., Fuchs, M.C., Fischer, M., Schmidt, C., 2016. The abanico plot: visualising chronometric data with individual standard errors 31, 12–18. [doi:10.1016/j.quageo.2015.09.003](http://dx.doi.org/10.1016/j.quageo.2015.09.003)
