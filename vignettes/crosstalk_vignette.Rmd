...
calc_MoransI_pseudo_p(object = vn_simulated) 
...
plot_Disc(object = vn_simulated_with_crosstalk, 
          main = "Simulated signal with crosstalk (measurement disc)")

calc_MoransI(object = vn_simulated_with_crosstalk)

calc_MoransI_pseudo_p(object = vn_simulated_with_crosstalk) 
...
for (i in 1:nrow(df_MoransI))
{
  ...
  df_MoransI$pseudo_p[i] <- 
    calc_MoransI_pseudo_p(object = vn_simulated_with_crosstalk, 
                          bo_suppress_warnings = TRUE) 


}
  

n_expected_I_if_no_spatial_autocorrelation <- calc_MoransI_expt_no_cor()
...
plot(x = df_MoransI$crosstalk, 
     ...
     ylim = range(
          pretty(x = c(df_MoransI$MoransI, n_expected_I_if_no_spatial_autocorrelation))
          ),
...
grid()
abline(h = n_expected_I_if_no_spatial_autocorrelation, 
       col = "purple")
...
The function `get_Neighbour` was until now used on the background, but we can explicitly call it to generate a dataframe with all positions which are rook connected to each other:
...
df_Neighbours <- get_Neighbour(object =  vn_simulated_with_holes)
...
```{r adjacent_grain_locations_show_disc, fig.dim = c(8, 5), out.width="800px"}
    ...
    plot_Disc(object = vn_simulated_with_holes,
...
df_Neighbours_with_diag <- get_Neighbour(object =  vn_simulated)
...
plot_Disc(object = vn_simulated,
          df_neighbour = df_Neighbours_with_diag,
          bo_show_neighbours = TRUE,
          bo_show_location_ids = TRUE)
...
To exclude all border effects, we can take out all border rows
...
df_neighbour <- get_Neighbour(bo_restrict_to_8x8 = TRUE)

plot_Disc(object = vn_values_to_show,
          bo_show_neighbours = TRUE,
...
calc_MoransI(object = 1:100,
      bo_return_inbetween_numbers = TRUE
      )



```
If the weights (and thus the spatial pattern under investigation) and the number of observations remain the same, this can be useful to understand what is happening. For example, if we add crosstalk, we can see that the population variance slightly decreases (the values are spatially smoothed) but the spatial autocorrelation strongly increases:
  ...
  data.frame(Case = c("Without crosstalk", "With crosstalk"),
             ...
             PopulationVar = 
               c(calc_MoransI(object = vn_simulated,
                 bo_return_inbetween_numbers = TRUE)$n_population_variance,
                 calc_MoransI(object = vn_simulated_with_crosstalk,
                 bo_return_inbetween_numbers = TRUE)$n_population_variance),
             SpatialAutoCor = 
               c(calc_MoransI(object = vn_simulated,
                 bo_return_inbetween_numbers = TRUE)$n_average_auto_correlation,
                 calc_MoransI(object = vn_simulated_with_crosstalk,
                 bo_return_inbetween_numbers = TRUE)$n_average_auto_correlation)
      )
