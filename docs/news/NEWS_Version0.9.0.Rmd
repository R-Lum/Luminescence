...
#### The big picture: `plot_DRCSummary()` 

You have been always wondering how the dose response curves would like if combined in a single plot? 
Now we have a solution: `plot_DRCSummary()` works directly on the output of the function `analyse_SAR.CWOSL()` and extracts the equations with the fitting paramaters for you can displays 
everything in a single plot. Of course, it works also on a `list` of objects created by 
`analyse_SAR.CWOSL()`.
