...
- All import functions will now always append the detector used to the
  ...
  consistency with what has always been done by
  [`read_XSYG2R()`](https://r-lum.github.io/Luminescence/reference/read_XSYG2R.md).
  The following functions have been modified to support this new
  behaviour:
  ...
  - `ExampleData.portableOSL` As a consequence of these changes, default
    ...
    when selecting curves based on their `recordType`
    ([\#1275](https://github.com/R-Lum/Luminescence/issues/1275),
    [\#1286](https://github.com/R-Lum/Luminescence/issues/1286)).

- Function
  [`read_XSYG2R()`](https://r-lum.github.io/Luminescence/reference/read_XSYG2R.md)
  will now prepend an underscore to the `recordType` slot of the
  `RLum.Data.Curve` object generated from an XSYG file for all objects
  after the first curve in a record. This will allow an easier way to
  differentiate the curves to analyse (for example, “OSL (UVVIS”) from
  those that are not useful in the analysis (for example, “\_OSL (NA)“)
  ([\#1276](https://github.com/R-Lum/Luminescence/issues/1276)).

- The `@info` slot returned by function
  [`read_XSYG2R()`](https://r-lum.github.io/Luminescence/reference/read_XSYG2R.md)
  no longer contains a `name` field: this has been replaced by
  `sequenceName`, to differentiate it from the newly-added `recordName`
  ([\#1239](https://github.com/R-Lum/Luminescence/issues/1239),
  [\#1387](https://github.com/R-Lum/Luminescence/issues/1387)).
...
- Functions
  ...
  return two extra columns, `SN_RATIO_LnLx` and `SN_RATIO_TnTx`, in
  their result object. This may disrupt code that relies on column
  indices instead of column names
  ([\#1281](https://github.com/R-Lum/Luminescence/issues/1281),
  [\#1330](https://github.com/R-Lum/Luminescence/issues/1330),
...
- [`normalise_RLum()`](https://r-lum.github.io/Luminescence/reference/normalise_RLum.md):
  This function enables a fast normalisation of `RLum.Data.Curve-class`,
  `RLum.Data.Spectrum-class`, `RLum.Data.Image-class` and their
  combination in `RLum.Analysis-class` objects. Normalisation was
  already possible for plotting (e.g.,
  `plot_RLum.Data.Curve(…, norm = TRUE)`), which uses an internal
  function called `.normalise_curves()` to do the job. With
  [`normalise_RLum()`](https://r-lum.github.io/Luminescence/reference/normalise_RLum.md)
  we expose this functionality to `RLum-class` objects and enable a
  convenient normalisation of values that might come in handy in same
  cases. Normalised values are only count values. Currently implemented
  is normalisation via keywords, such as `"max"`, `"min"`, `"first"`,
  `"last"`, `"huot"`, `"intensity"`. Additionally, the function supports
  normalisation to a positive number
  ([\#1250](https://github.com/R-Lum/Luminescence/issues/1250) by
  [@RLumSK](https://github.com/RLumSK),
  [\#1300](https://github.com/R-Lum/Luminescence/issues/1300)).

- [`read_BINXLOG2R()`](https://r-lum.github.io/Luminescence/reference/read_BINXLOG2R.md):
  This function enables the salvaging of log files created by a Risø
  reader into R as `RLum.Analysis-class`. It also allows for exporting
  these files into a BIN/BINX format. While the function has been tested
  on a few examples, it is important to note that log files are not a
  ...
  still available. Supported log formats are TL, OSL, and IRSL curves.
  All other formats are ignored
  ([\#1241](https://github.com/R-Lum/Luminescence/issues/1241) by
  [@RLumSK](https://github.com/RLumSK); thanks to Michael Kenzler for
...
- Functions `github_branches()`, `github_commits()`, `github_issues()`,
  ...
  ([\#1242](https://github.com/R-Lum/Luminescence/issues/1242)).
...
- The `signal_integral` argument was silently ignored
  ([\#1428](https://github.com/R-Lum/Luminescence/issues/1428)).
...
- The function used to output a suppressed error message instead of
  validating the input ahead of that
  ([\#1256](https://github.com/R-Lum/Luminescence/issues/1256)).
...
- The function has been updated to use
  ...
  This should not have any visible difference other than the renaming of
  two arguments: `output.plot` has been renamed to `plot_drc`, while
  `output.plotExtended` has been renamed to `plot_extended`. The older
  names will still work but generate a deprecation warning
  ([\#1244](https://github.com/R-Lum/Luminescence/issues/1244)).
...
- Arguments `signal.integral`, `background.integral`,
  ...
  work but generate a deprecation warning
  ([\#1290](https://github.com/R-Lum/Luminescence/issues/1290)).

- The function no longer crashes if `aliquot_range` specifies
  non-positive indices or if `source_doserate = 0`
  ([\#1424](https://github.com/R-Lum/Luminescence/issues/1424),
  [\#1426](https://github.com/R-Lum/Luminescence/issues/1426)).
...
- The plots were affected by a bug in
  ...
  so that the output was unnecessarily split over two pages
  ([\#1247](https://github.com/R-Lum/Luminescence/issues/1247)).
...
- The `slide$squared_residuals` field of the `@data` slot in the
  ...
  matrix, but a plain numeric vector
  ([\#1254](https://github.com/R-Lum/Luminescence/issues/1254)).
...
- The function crashed if called on an empty list
  ([\#1372](https://github.com/R-Lum/Luminescence/issues/1372)).
...
- If `mode = "alternate"`, the dose value is no longer reset
  ...
  [`fit_DoseResponseCurve()`](https://r-lum.github.io/Luminescence/reference/fit_DoseResponseCurve.md))
  because it modified the first dose point and this required additional
  handling afterwards. Please keep in mind that you have to redefine the
  recuperation reference in such a case: this is wanted and not a bug
  (fixed in commit 9ff29bf).
...
- The order of columns in the `rejection.criteria` data frame that is
  ...
  aliquot also after merge operations
  ([\#1172](https://github.com/R-Lum/Luminescence/issues/1172)).
...
- The function warns if the most common curve type (which is the one
  ...
  to analyze
  ([\#1269](https://github.com/R-Lum/Luminescence/issues/1269)).
...
- A new signal-to-noise ratio rejection criterion has been added, with
  ...
  parameter in the `rejection.criteria` list
  ([\#1284](https://github.com/R-Lum/Luminescence/issues/1284)).
...
- When rejection criteria are set to `NA`, in the plot we now only show
  ...
  ([\#1318](https://github.com/R-Lum/Luminescence/issues/1318)).
...
- It is now possible to consider value uncertainties in the computation
  ...
  is disabled by default to preserve existing behaviour
  ([\#1325](https://github.com/R-Lum/Luminescence/issues/1325)).

- The function now supports setting `background_integral = NA`, in which
  case no background integral is subtracted
  ([\#1385](https://github.com/R-Lum/Luminescence/issues/1385); thanks
...
- When `OSL.component` is specified, the function now checks that the
  ...
  warning message when that is not the case
  ([\#1464](https://github.com/R-Lum/Luminescence/issues/1464)).
...
- Curves with `recordType` starting with `_` are automatically removed.
  ...
  ([\#1468](https://github.com/R-Lum/Luminescence/issues/1468)).

#### `analyse_SAR.TL()`

- Argument `signal.integral` has been renamed to `signal_integral`. The
  older name will still work but generate a deprecation warning
  ([\#1290](https://github.com/R-Lum/Luminescence/issues/1290)).
...
- The function no longer produces an invalid object when the
  ...
  ([\#1402](https://github.com/R-Lum/Luminescence/issues/1402)).
...
- The `RLum.Results` returned now contains a `$data` field to store the
  ...
  ([\#1455](https://github.com/R-Lum/Luminescence/issues/1455)).

#### `calc_CobbleDoseRate()`

- Argument `input` has been renamed to `object`. The older name will
  still work but generates a deprecation warning
  ([\#1434](https://github.com/R-Lum/Luminescence/issues/1434)).

#### `calc_CommonDose()`

- The function incorrectly disallowed setting `sigmab > 1` when
  `log = FALSE` due to excessively strict input validation
  ([\#1343](https://github.com/R-Lum/Luminescence/issues/1343)).
...
- The function didn’t allow inputs such as `n.components = 1:3`,
  ...
  in v1.1.0, was unnecessarily stringent and has been reverted
  ([\#1458](https://github.com/R-Lum/Luminescence/issues/1458)).
...
- The De estimates produced when `bootstrap = TRUE` were not transformed
  back to the original scale before being returned to the user
  ([\#1351](https://github.com/R-Lum/Luminescence/issues/1351)).
...
- The function now set non-positive De values to `NA` when `log = TRUE`,
  ...
  ([\#1313](https://github.com/R-Lum/Luminescence/issues/1313)).
...
- The bootstrap results stored in the `@data$bootstrap$pairs$gamma`
  ...
  values to be stored in the log scale
  ([\#1345](https://github.com/R-Lum/Luminescence/issues/1345)).
...
- The function now stores the bootstrap results in the `summary` slot
  ...
  terminal also when using bootstrap
  ([\#1336](https://github.com/R-Lum/Luminescence/issues/1336); thanks
  to [@feldsparlover](https://github.com/feldsparlover) and Arindam
  Biswas for reporting and to Marijn van der Meij for checking the
  implementation).
...
- The function crashed when bootstrapping with `bs.N = 1`. Now the
  ...
  fitting ([\#1355](https://github.com/R-Lum/Luminescence/issues/1355)).
...
- The computation of frequencies of bootstrap replicates has been fixed
  ...
  ([\#1374](https://github.com/R-Lum/Luminescence/issues/1374)).
...
- The data frame returned by the function now contains two additional
  ...
  consistency with the output produced by
  [`calc_OSLLxTxRatio()`](https://r-lum.github.io/Luminescence/reference/calc_OSLLxTxRatio.md)
  ([\#1330](https://github.com/R-Lum/Luminescence/issues/1330)).
...
- The function can now accept lists for its `Lx.data` and `Tx.data`
  ...
  all records
  ([\#1299](https://github.com/R-Lum/Luminescence/issues/1299)).
...
- The function now supports setting `signal_integral` to a chosen set of
  ...
  ([\#1375](https://github.com/R-Lum/Luminescence/issues/1375)).
...
- The function crashed if the `Age_range` argument contained missing
  values ([\#1378](https://github.com/R-Lum/Luminescence/issues/1378)).
...
- The function now checks if the results of the approximation step are
  ...
  range, which can happen if the input data is misspecified
  ([\#1295](https://github.com/R-Lum/Luminescence/issues/1295)).
...
- The interpolation of attenuation values has been corrected so that it
  ...
  is provided, as long as it’s in the valid range
  ([\#1444](https://github.com/R-Lum/Luminescence/issues/1444); thanks
  to Christina Neudorf for reporting).
...
- The `dose_rate` argument is now better validated
  ([\#1439](https://github.com/R-Lum/Luminescence/issues/1439)).
...
- The function crashed if used on an object resulting from a subset
  ...
  ([\#1415](https://github.com/R-Lum/Luminescence/issues/1415)).

- The function now exits cleanly instead of crashing if more than one
  input filenames are provided
  ([\#1446](https://github.com/R-Lum/Luminescence/issues/1446)).

#### `extract_IrradiationTimes()`

- The function no longer crashes if the input object contains no
  `position` field in its info object
  ([\#1466](https://github.com/R-Lum/Luminescence/issues/1466)).
...
- The C++ implementation of the GOK function has been tweaked to avoid
  ...
  ([\#1277](https://github.com/R-Lum/Luminescence/issues/1277)).

- The fit message reported is now also stored in the `@info` slot of the
  object returned by the function
  ([\#1362](https://github.com/R-Lum/Luminescence/issues/1362)).
...
- The function now checks that the number of data points provided is
  large enough to fit the requested number of components
  ([\#1448](https://github.com/R-Lum/Luminescence/issues/1448)).
...
- An internal optimization increased the performance of the function by
  ...
  of `RLum.Analysis` object
  ([\#1271](https://github.com/R-Lum/Luminescence/issues/1271)).

#### `merge_RLum.Risoe.BINfileData()`

- Argument `input.objects` has been renamed to `objects`. The older name
  will still work but generates a deprecation warning
  ([\#1434](https://github.com/R-Lum/Luminescence/issues/1434)).

#### `merge_RLum.Data.Spectrum()`

- The function no longer warns unnecessarily if the RLum.Data.Spectrum
  object contains no column names
  ([\#1450](https://github.com/R-Lum/Luminescence/issues/1450)).
...
- When the input data is an `RLum.Results` object generated by
  ...
  one or more lines (and corresponding labels) for the relevant results
  will be automatically drawn, unless explicitly suppressed by setting
  `line = NA`
  ([\#1452](https://github.com/R-Lum/Luminescence/issues/1452)).

#### `plot_DetPlot()`

- When operating on a list with the default setting of
  `multicore = TRUE`, the function limits the size of the parallel
  cluster to the number of list elements to process
  ([\#1258](https://github.com/R-Lum/Luminescence/issues/1258)).
...
- The scaling of the subtitle by `cex` didn’t account of the `par.local`
  ...
  [`plot_RLum.Analysis()`](https://r-lum.github.io/Luminescence/reference/plot_RLum.Analysis.md)
  ([\#1347](https://github.com/R-Lum/Luminescence/issues/1347)).

#### `plot_RLum.Data.Spectrum()`

- The `norm` argument is no longer limited to just `"min"` and `"max"`
  but now supports all options provided by
  [`normalise_RLum()`](https://r-lum.github.io/Luminescence/reference/normalise_RLum.md)
  ([\#1303](https://github.com/R-Lum/Luminescence/issues/1303)).
...
- The function now returns `NULL` (instead of producing an empty plot or
  ...
  ([\#1305](https://github.com/R-Lum/Luminescence/issues/1305)).
...
- Setting `log = "z"` will produce a warning if the data contains
  ...
  plot types
  ([\#1307](https://github.com/R-Lum/Luminescence/issues/1307)).
...
- The length of the ticks for the rugs is now scaled by the size of
  ...
  ([\#1470](https://github.com/R-Lum/Luminescence/issues/1470); thanks
  to [@feldsparlover](https://github.com/feldsparlover) for reporting).
...
- If the error column contains zero values, they are reset to the
  ...
  ([\#1314](https://github.com/R-Lum/Luminescence/issues/1314)).
...
- The weighted median used to compute the central values when
  ...
  incorrectly used the error
  ([\#1323](https://github.com/R-Lum/Luminescence/issues/1323)).

#### `read_BIN2R()`

- The function could crash if called with
  [`do.call()`](https://rdrr.io/r/base/do.call.html) and some
  particularly odd inputs
  ([\#1260](https://github.com/R-Lum/Luminescence/issues/1260)).

- The function now throws an error instead of returning `NULL` when a
  file with the wrong extension is read
  ([\#1393](https://github.com/R-Lum/Luminescence/issues/1393)).
...
- The function struggled with variable header lengths
  ([\#1262](https://github.com/R-Lum/Luminescence/issues/1262); thanks
  to [@andrzejbluszcz](https://github.com/andrzejbluszcz) for
  reporting).

- The function will no longer crash if the data point entries have more
  than four columns (thanks to
  [@andrzejbluszcz](https://github.com/andrzejbluszcz) for reporting).
...
- The import of binary `.DAT` files has been overhauled and made more
  ...
  in ASCII files
  ([\#1263](https://github.com/R-Lum/Luminescence/issues/1263),
  [\#1266](https://github.com/R-Lum/Luminescence/issues/1266); thanks to
  [@andrzejbluszcz](https://github.com/andrzejbluszcz) for contributing
  code and datasets).

- Trying to import multiple files by specifying a directory name failed
  because the generated path missed a directory separator
  ([\#1264](https://github.com/R-Lum/Luminescence/issues/1264)).

#### `read_PSL2R()`

- The function now returns `NULL` instead of throwing an error if a
  directory is provided and no PSL files are found within it
  ([\#1393](https://github.com/R-Lum/Luminescence/issues/1393)).

#### `read_RF2R()`

- The function now throws an error instead of returning `NULL` if a list
  containing elements other than character strings is provided
  ([\#1393](https://github.com/R-Lum/Luminescence/issues/1393)).

#### `read_SPE2R()`

- The function now throws an error instead of returning `NULL` if the
  filename provided does not exist
  ([\#1393](https://github.com/R-Lum/Luminescence/issues/1393)).
...
- Attributes from the record level are now all included in the `@info`
  ...
  ([\#1239](https://github.com/R-Lum/Luminescence/issues/1239); thanks
...
- The function used to add spurious `NULL` records when an invalid
  ...
  displaying the object to the terminal
  ([\#1380](https://github.com/R-Lum/Luminescence/issues/1380)).
...
- The function reports an helpful error instead of crashing if the given
  grains and positions table produces an empty selection
  ([\#1412](https://github.com/R-Lum/Luminescence/issues/1412)).
...
- The internal function `.normalise_curve()` received support to
  ...
  [`plot_RLum.Analysis()`](https://r-lum.github.io/Luminescence/reference/plot_RLum.Analysis.md)
  and the `plot_RLum.Data.*()` functions
  ([\#1250](https://github.com/R-Lum/Luminescence/issues/1250),
  [\#1300](https://github.com/R-Lum/Luminescence/issues/1300)).
