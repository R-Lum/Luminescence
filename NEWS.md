...
# Changes in version 1.1.2.9000-37 (2026-02-06)
...
- The show method for an `RLum.Analysis-class` object would fail if the
  record slot contains `NULL` elements. This should not happen by
  design, however, it may happen if the user uses, for instance,
  `remove_RLum()` to strip records that do no exists. Regardless, the
  show method should not fail; fixed.
