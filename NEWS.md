...
# Changes in version 1.1.2.9000-38 (2026-03-08)
...
- `read_BINXLOG2R()`: This function enables you to salvage log files
  ...
  note that log files are not a well-defined data format. This function
  is intended for use in cases where you have a corrupted BIN/BINX file
  but a complete log file is still available. Supported log formats are
  TL, OSL, and OSL curves. All other formats are ignored (#1241 by
  @RLumSK; thanks to Michael Kenzler for bringing this to our
  attention).
...
- The new `od_counts` argument allows to activate an alternative
  approach to Galbraith’s for calculation of overdispersion (#1390;
  thanks to @andrzejbluszcz for the mathematical description).
