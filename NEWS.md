...
# Changes in version 0.9.25.9000-81 (2024-11-03)
...
- The function crashed contained for single grain data that sometimes
  cause a lot of `NaN` values during log conversions required for
  estimating the start parameters. The result was the error
  `object 'b.MC' not found` (#374; fixed)
- If `fit.method = "EXP+LIN"`, the function stuck sometimes in an
  endless loop in cases where the number of parameters was larger or
  equal the number of data points. To avoid this, we replaced `nls()`
...
- The function gained a new argument `file_input` that enables the
  ...
  way, already available DRAC files cannot only be used by conveniently
  modified without a pass to `use_DRAC()` first.
- The template contained a couple of minor mistakes that got fixed along
  ...
  effect.
- The function gain knowledge about the conversion factors by Cressswell
  et al. (2018) `Cresswelletal2018`. Please ensure that the server
...
- **Potentially breaking old code!**: if `cleanup = TRUE` the result was
  not consistent and produced a mixture of `RLum.Results-class` and
  `RLum.Analysis-class`/`Risoe.BINFileData-class` objects in particular
  when run over a list, this was not expected from what is written in
  the manual. Now the function  
...
- The function doesn’t crash anymore when applied to an empty list
  (#365, fixed in \#366).
- Silence the very talkative terminal even if nothing was removed from a
  record for `cleanup = TRUE`.
- Provide better terminal feedback if the selection resulted in an empty
  record.
...
- `get_get_DRAC_references()` now works also on a `DRAC.list` got
  updated references.
