...
## Changes in version 0.9.8.9000-17 (2020-02-21)
...
#### `analyse_Al2O3C_Measurements()`
  ...
  - The function gained a new argument `plot_onePage`. If set to `TRUE`
    plot output is gathered in one single plot window instead of three.
...
#### `analyse_FadingMeasurements()`
  ...
  - The function did not work properly for less than five data points
    and stopped with `Error in rep(rgb(0, 0, 0, 0.3),
    length(TIMESINCEIRR) - 5)` (thanks to Annette Kadereit for flagging
    this issue)

#### `analyse_pIRIRSequence()`

  - The one page plot option (`plot.single = TRUE`) introduced for
    `analyse_SAR.CWOSL()` in December 2019 broke the plot output of
    `analyse_pIRIRSequence()`. No one complained, however, it was a bug
...
#### `fit_OSLLifeTimes()`
  ...
  - Function now handles the `log = "x"`, `log = "y"` and `log = "xy"`
    options better.
  - In the self-call mode (imput is `RLum.Analysis-class` object), all
