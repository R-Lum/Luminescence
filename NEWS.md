...
# Changes in version 0.9.26.9000-110 (2025-02-20)
...
- `sort_RLum()`: Allows to sort the records of an `RLum.Analysis` object
  according to a given slot or info element (#528, fixed in \#571).
...
- If the info element `startDate` was missing an in the irradiation
  curve the function crashed unexpectedly; fixed.
...
- Remove support for `tcl` and `tck` arguments via `...`, they were
