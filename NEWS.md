...
## Changes in version 0.9.9.9000-36 (2020-11-26)
...
-   The default values for the `rejection.criteria` is now `list()`
    instead of `NULL`. As before, the default values uses the internal
    set rejection criteria and this change should not affect the
    behaviour of the function, but it allowed use to greatly simplify
    the underlying code.
-   The function did not check correctly for all record types, under
    certain circumstances, some of the non-support curves could slip
    through; corrected.
...
-   As implemented by `calc_OSLLxTxRatio()` the function now supports
    `NA` values for the integrals. If this happens, sum curves are
    taken. This option enables a bunch of new possibilities to test more
    experimental protocols without modifying the function.
-   Following the `NA` support, all integral arguments are now set to
    `NA` by default. To avoid user errors, a warning is thrown each time
    the function runs with integral limits set to `NA`.
-   The identification of the allowed curves was further improved to
    cover cases where users want to provide experimental OSL curves.
    Allowed are combinations where `OSL` with additional letters
    `[a-zA-Z]`. Everything else is, as before, ignored.
-   The function now supports the input from the upcoming R package
    `'OSLdecomposition'` by Dirk Mittelstrass. For this a new argument
    was added `OSL.component`.
...
-   The function now supports `NA` as input for the `signal.integral`
    ...
    component data that were already extracted and do not need anymore
    integral limits.
-   The output of the function is not anymore invisible, but a normal
    return. This should have no further effect.
...
-   Users sometimes complained about large uncertainties produced by the
    ...
    function. Means, you can run any statistic you prefer to best
    estimate they uncertainties on the *D*<sub>*e*</sub>.

#### `read_Daybreak()`

-   The function threw a lot of warnings for the `*.DAT` import after
...
-   The automated tests now use consistently `testthat` &gt;= 3.0.0.
    Naturally this has no use visible effects, however, we found a
