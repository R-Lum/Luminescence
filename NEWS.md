...
# Changes in version 1.1.2.9000-38 (2026-02-12)
...
- The function now throws an error instead of returning `NULL` when a
  file with the wrong extension is read (#1395).
...
- The function now returns `NULL` instead of throwing an error if a
  directory is provided and no PSL files are found within it (#1395).

### `read_RF2R()`

- The function now throws an error instead of returning `NULL` if a list
  containing elements other than character strings is provided (#1395).

### `read_SPE2R()`

- The function now throws an error instead of returning `NULL` if the
  filename provided does not exist (#1395).
