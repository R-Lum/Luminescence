...
# Changes in version 1.1.0.9000-12 (2025-09-01)
...
- For `mode = "interpolation"`, the object returned contains an
  additional `De.raw` column, which stores the value of the raw De value
  as computed by the fitting function. The only difference between the
  `De` and `De.raw` columns, is that the first sets the De to `NA` if
  negative, while the latter doesn’t. This can be helpful when testing
  that the De values from a null are close to zero; for general
  applications, this column should be ignored (#957).
