...
## Changes in version 0.9.11.9000-58 (2021-04-01)
...
-   `convert_SG2MG()` Convert single-grain data to multiple-grain data
    by adding singles from disc. Works only for files created by Risø
    TL/OSL readers.
...
-   The function caused a crash in `analyse_SAR.CWOSL()` for very small
    *D*<sub>*e*</sub> error values returning a user unfriendly error
...
-   Add support for the Lambert W dose-response curve suggested by
    Pagonis et al. (2020) (thanks to @vpagonis for developing the math)

#### `read_BIN2R()`

-   Only BIN/BINX-file version 3 and 4: if the numbers of runs exceeded
    128, the number became negative; fixed.

### Internals

-   `'lamW'` add as new package dependency. This is needed to support
