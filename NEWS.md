...
## Changes in version 0.9.0 (2019-04-13)
...
#### `analyse_SAR.CWOSL()`
  ...
  - If the function was used in a self-call mode (object of type
    ...
    the arguments such as `signal.integra.min` had not been expanded
...
### Internals
  ...
  - Internal changes to address changes in R-devel checking logical
    argumnets with length more than one (e.g., `if(is.na(x))` returned
  ...
  - `src/Makevars` file removed to prevent unconditional stripping in
    ...
    of the packagee increased in size on some platforms.
