...
# Changes in version 0.9.25.9000-94 (2024-11-12)
...
- The function now shows a warning and sets `plot = FALSE` when option
  `plot_singlePanels = FALSE` is set but the device size is too small.
  This should prevent “Figure margins too large” errors.
...
- The function crashed for single grain data that sometimes cause a lot
  ...
  (#374; fixed)
- If `fit.method = "EXP+LIN"`, the function could get stuck in an
